...
export default function Reviews() {
    return (
        <section id="resenas" className="py-10 md:py-14 xl:py-16 bg-white relative">
            <div className="container mx-auto px-4 md:px-6 max-w-7xl">
                ...
                <div className="flex overflow-x-auto pb-8 -mx-4 px-4 snap-x snap-mandatory md:grid md:grid-cols-3 md:gap-6 md:overflow-visible md:pb-0 md:mx-0 md:px-0 scrollbar-hide items-stretch">
                    {reviews.map((review) => (
                        <div key={review.id} className="w-[85vw] flex-shrink-0 snap-center md:w-auto md:flex-shrink bg-bg-light p-6 rounded-2xl shadow-sm border border-gray-100 flex flex-col h-full relative group hover:-translate-y-1 hover:shadow-md transition-all duration-300 mr-4 md:mr-0">
                            <Quote className="absolute top-4 right-4 text-primary/20 w-10 h-10 group-hover:text-primary/40 transition-colors" />
                            ...
                            <p className="text-accent text-sm mb-6 flex-grow italic relative z-10 leading-relaxed">
                                "{review.text}"
                            </p>

                            <div className="mt-auto">
                                <hr className="border-accent/10 mb-4" />
                                <h4 className="font-bold text-accent font-serif text-base">{review.name}</h4>
                                <p className="text-xs text-accent/70">{review.location}</p>
                            </div>
                        </div>
                    ))}
                </div>
